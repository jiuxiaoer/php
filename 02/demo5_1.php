...
// 必须让子进程先结束 不然 子进程 会被1号进程接管变孤儿进程
// 如果子进程先结束 父进程后结束 一般来说是正常的 但是父进程 应该回收 子进程
// 子进程结束时[会生成一些数据 状态码 需要被父进程回收]
if (0==$pid){
    //子进程
    fprintf(STDOUT, "我是子进程我开始运行了%d\n",posix_getpid());
    fprintf(STDOUT,"子：xpid=%d,text=%s\n",$pid,$text);
}else{
    //父进程
    sleep(2);//让父进程先睡眠  子进程运行 防止成为孤儿
    fprintf(STDOUT, "我是父进程我睡了两秒%d\n",posix_getpid());
    fprintf(STDOUT,"父：xpid=%d,text=%s\n",$pid,$text);
}
